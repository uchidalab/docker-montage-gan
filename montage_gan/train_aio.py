...
import dnnlib
from custom import training_loop_aio
...
def setup_training_loop_kwargs(
        ...
        img_snap=None,  # Image snapshot interval: <int>, default = 1 ticks
        net_snap=None,  # Network snapshot interval: <int>, default = 50 ticks
    ...
    if net_snap is None:
        net_snap = 50
    assert isinstance(net_snap, int)
    if net_snap < 1:
        raise UserError('--net-snap must be at least 1')
    args.network_snapshot_ticks = net_snap
    ...
    args.local_D_kwargs.epilogue_kwargs.mbstd_group_size = args.global_D_kwargs.epilogue_kwargs.mbstd_group_size = spec.mbstd

    args.local_G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0, 0.99], eps=1e-8)
    args.local_D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0, 0.99], eps=1e-8)
    args.global_G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0, 0.99], eps=1e-8)
    args.global_D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=spec.lrate, betas=[0, 0.99], eps=1e-8)

    args.loss_kwargs = dnnlib.EasyDict(class_name='custom.loss_aio.MontageGANLoss', global_r1_gamma=spec.gamma,
                                       r1_gamma=spec.gamma)
    ...
    if cfg == 'cifar':
        ...
        args.local_D_kwargs.architecture = 'orig'  # disable residual skip connections
    ...
    if freezed is not None:
        ...
        args.local_D_kwargs.block_kwargs.freeze_layers = freezed
    ...
    if fp32:
        args.local_G_kwargs.num_fp16_res = args.local_D_kwargs.num_fp16_res = 0
        args.local_G_kwargs.conv_clamp = args.local_D_kwargs.conv_clamp = None

    if nhwc is None:
        nhwc = False
    assert isinstance(nhwc, bool)
    if nhwc:
        args.local_G_kwargs.fp16_channels_last = args.local_D_kwargs.block_kwargs.fp16_channels_last = True
...
def subprocess_fn(rank, args, temp_dir):
    ...
    training_loop_aio.training_loop(rank=rank, **args)
...
@click.option('--img-snap', help='Image snapshot interval [default: 1 tick]', type=int, metavar='INT')
@click.option('--net-snap', help='Network snapshot interval [default: 50 ticks]', type=int, metavar='INT')
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    try:
        run_desc, args, res = setup_training_loop_kwargs(**config_kwargs)
