...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        if use_noise:
            self.register_buffer('noise_const', torch.randn(
                [self.resolution * self.init_res[0] // 4, self.resolution * self.init_res[1] // 4]))
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        misc.assert_shape(x, [None, self.weight.shape[1], in_resolution * self.init_res[0] // 4,
        ...
        if self.use_noise and noise_mode == 'random':
            noise = torch.randn(
                [x.shape[0], 1, self.resolution * self.init_res[0] // 4, self.resolution * self.init_res[1] // 4],
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        if in_channels == 0:
            self.const = torch.nn.Parameter(
                torch.randn([out_channels, resolution * self.init_res[0] // 4, resolution * self.init_res[1] // 4]))
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            misc.assert_shape(x, [None, self.in_channels, self.resolution * self.init_res[0] // 8,
        ...
        # ToRGB.
        if img is not None:
            misc.assert_shape(img, [None, self.img_channels, self.resolution * self.init_res[0] // 8,
                                    self.resolution * self.init_res[1] // 8])
    ...
    def __init__(self,
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None,
                                          **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, init_res=init_res,
