...
dataset_config = {
    "name": "global",
    "debug": False,
    # Result cache from dataset_stat, skip the calculation if defined
...
def dataset_stat(src, conv_config_index=2):
    ...
    from custom_utils.calc_res import find_min_res_combination, calc_res_combination

    acceptable_res_combination = calc_res_combination(conv_config_index)
    ...
    for i in layers.items():
        ...
        # target res
        target_w, target_h = find_min_res_combination((max_w, max_h), acceptable_res_combination,
                                                      conv_config_index)
...
class DatasetAIO(Dataset):
    def __init__(self,
                 ...
                 conv_config_index=2,
                 **kwargs  # Ignore other unsupported kwargs.
                 ):
    ...
    @property
    def init_res(self):
        return calc_init_res(self.image_shape[1:], conv_config_index=self.conv_config_index)[0]

    def res_log2_layer(self, layer_name):
        stat_layer = self.layer_stats[layer_name]
        layer_size = [stat_layer["target_height"], stat_layer["target_width"]]
        return calc_init_res(layer_size, conv_config_index=self.conv_config_index)[2]

    def init_res_layer(self, layer_name):
        stat_layer = self.layer_stats[layer_name]
        layer_size = [stat_layer["target_height"], stat_layer["target_width"]]
        return calc_init_res(layer_size, conv_config_index=self.conv_config_index)[0]
    ...
    def target_res_layer(self, layer_name):
        stat_layer = self.layer_stats[layer_name]
        layer_size = [stat_layer["target_height"], stat_layer["target_width"]]
        return layer_size
...
if __name__ == "__main__":
    ...
    root_path = "../../data/global"

    conv_config_index = 3
    dataset = DatasetAIO(root_path, conv_config_index=conv_config_index)
    dataloader = DataLoader(dataset, batch_size=32, shuffle=True)
    print("Dataset length:", len(dataset))
    # Draw a sample batch from DataLoader
    data = next(iter(dataloader))
    print("Data shape:", data.shape)
    save_image(data.flatten(0, 1), "dataset_test.png", nrow=9)
    print("Layers:")

    for layer in dataset.layer_names:
        print(
            f"{layer}. From init res {dataset.init_res_layer(layer)} -> target res {dataset.target_res_layer(layer)},",
            f"2x up-sampling for {dataset.res_log2_layer(layer) - conv_config_index} times.")
