...
from fukuwarai.networks import STNv2c as STN  # Tanh variant
...
while not break_flag:
    for i, data in enumerate(tqdm(dataloader, total=len(dataloader))):
        ...
        pseudo_fake = normalize_minus11(pseudo_fake)  # [-1,1]
        x_fake, _ = stn(pseudo_fake)
        rendered = renderer(x_fake)
        z_outputs = D(rendered.detach())
        ...
        rendered = renderer(x_fake)
        alpha_blended = alpha_composite(normalize_zero1(x_fake.detach()))  # [0,1]
        ...
        renderer_loss = loss(normalize_zero1(rendered), alpha_blended)

        z_outputs = D(rendered)
        # grid = make_grid(normalize_zero1(rendered), padding=0)  # [0,1]
        ...
        if step % config["log_term"] == 0 or is_last:
            ...
            x_fake, theta = stn(pseudo_fake)
            rendered = renderer(x_fake)
            grid = make_grid(normalize_zero1(rendered), padding=0)
            writer.add_image("G/rendered", grid, step)
            stn.train()
